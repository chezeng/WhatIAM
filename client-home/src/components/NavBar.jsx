...
const NavBar = () => {
    ...
    return (
        <nav className={`z-10 w-screen h-20 flex items-center justify-between bg-black fixed transition-transform duration-300 ${visible ? 'transform translate-y-0' : 'transform -translate-y-full'}`}>
          <h1 onClick={() => scrollToSection('intro')} className="text-white text-xl md:text-2xl ml-10 font-bold cursor-pointer hover:text-blue-500 hover:scale-110 transition duration-100 ease-out" > 
            Cheng Zeng
          </h1>
          ...
          <div className={`sm:hidden absolute text-center flex-col items-center space-y-3 top-24 -right-24 w-28 px-5 py-3 rounded-xl bg-black border-white border-2 transition transform duration-300 ease-in-out ${!toggle ? 'translate-x-20' : '-translate-x-full'}`}>
            <p className='text-white font-bold cursor-pointer hover:text-blue-500 transition' onClick={() => scrollAndToggle('about') }>About</p>
            <p className='text-white font-bold cursor-pointer hover:text-blue-500 transition' onClick={() => scrollAndToggle('projects')}>Projects</p>
            <p className='text-white font-bold cursor-pointer hover:text-blue-500 transition' onClick={() => scrollAndToggle('contact')}>Contact</p>
          </div>        
        </nav>
    );
};
